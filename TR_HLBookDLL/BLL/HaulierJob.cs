...
namespace FM.TR_HLBookDLL.BLL
{
    public class HaulierJob : VehicleJob
    {
         ...
         */
        public string PartnerJobNo { get; set; } // 20160715 - gerry renamed from
        ...
        public bool ValidateAddHaulierJobHeader()
        {
            ...
            */
            ...
            //TODO add validation if reference number already exist
            ...
            if (base.ValidateJobHeader() == true)
            {
                if (ValidateAddHaulierJobHeader() == true)
                {
                    ...
                    try
                    {
                        ...
                        Dictionary<string, Decimal> dictContainerQty = GetContainerQtyByJobID_New(this.JobID, con, tran);//20170726
                        TransportFacadeOut.AddTptJobToSpecialJobDetail(haulierJob, con, tran, prefix, dictContainerQty);
                        ...
                        AuditLog auditLog = new AuditLog(haulierJob.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Header", 0, "ADD");
        ...
        public string AddHaulierJobHeader(string formName, string user)
        {
            ...
            using (SqlConnection dbCon = new SqlConnection(strDbCon))
            {
                ...
                try
                {
                    ...
                    ////20151028 - gerry added new audit log
                    AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Header", 0, "ADD");
                    auditLog.WriteAuditLog("JobNumber", this.JobNo, string.Empty, dbCon, dbTran);
        ...
        public bool EditHaulierJobHeader(string formName, SUBCONCHANGES subConChanges, string user)
        {
            ...
            if ((TptDeptCode == OldTptDeptCode) | ((TptDeptCode != OldTptDeptCode) & (CanChangeTptDeptCode())))
            {
                ...
                try
                {
                    ...
                    //20160601 - gerry allow to edit completed job but only remarks field wil be available
                    ...
                    string prefix = "";
                    prefix = HaulierJobDAL.GetPrefix();
                    Dictionary<string, Decimal> dictContainerQty = GetContainerQtyByJobID_New(this.JobID, con, tran);
                    TransportFacadeOut.EditTptJobToSpecialJobDetail(this, con, tran, prefix, dictContainerQty);
                    #region Old auditlog
                    ...
                    //20151028 - gerry added new audit log
                    AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Header", 0, "EDT");
        ...
        public bool DeleteHaulierJob(string formName, string user)
        {
            ...
            if (ValidateDeleteJob() == true)
            {
                ...
                try
                {
                    ...
                    //20151028 - gerry added new audit log
                    AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, formName, 0, "DEL");
            ...
            try
            {
                ...
                if (haulierJobTrip.ValidateAddHaulierJobTrip())
                {
                    ...
                    if (HaulierJobDAL.AddHaulierJobTrip(this, haulierJobTrip, con, tran) == true)
                    {
                        //20170726
                        Dictionary<string, Decimal> dictContainerQty = GetContainerQtyByJobID_New(this.JobID, con, tran);
                        TransportFacadeOut.EditTptJobToSpecialJobDetail(this, con, tran, HaulierJobDAL.GetPrefix(), dictContainerQty);
                        // Create a JobTripState and write to DB
                        JobTripState jobTripState = new JobTripState(1, haulierJobTrip.TripStatus, this.JobDateTime, "Trip added by " + user, true);
                        ...
                        ////20151028 - gerry added new audit log
                        AuditLog auditLog = new AuditLog(haulierJobTrip.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Trip", haulierJobTrip.Sequence, "ADD");
                        auditLog.WriteAuditLog("SeqNo", haulierJobTrip.Sequence.ToString(), string.Empty, con, tran);
                        tran.Commit();
                    }
                }
            }
        ...
        public bool AddHaulierJobTrip(HaulierJobTrip haulierJobTrip, string frmName, string user, SqlConnection con, SqlTransaction tran, bool isTransferJob = false)
        {
            ...
            try
            {
                ...
                if (haulierJobTrip.LegType == Leg_Type.OneLeg && !isTransferJob)
                    CheckDoubleContainerSealNos(haulierJobTrip, con, tran);
                ...
                if (haulierJobTrip.ValidateAddHaulierJobTrip(isTransferJob))
                {
                    ...
                    {
                        // Create a JobTripState and write to DB
                        JobTripState jobTripState = new JobTripState(1, JobTripStatus.Booked, this.JobDateTime, "Trip added by " + user, true);
                        haulierJobTrip.AddJobTripState(jobTripState, con, tran);
            ...
            try
            {
                ...
                if (haulierJobTrip.ValidateEditHaulierJobTrip())
                {
                    ...
                    if (haulierJobTrip.JobTripStates[haulierJobTrip.JobTripStates.Count - 1].Status != haulierJobTrip.TripStatus)
                    {
                        ...
                        JobTripState jobTripState = new JobTripState(1, haulierJobTrip.TripStatus, this.JobDateTime, " new state" + user, true);
                        haulierJobTrip.AddJobTripState(jobTripState, con, tran);
                    }
                    ...
                    if (frmName != MULTI_LEG_FORM)
                    {
                        ...
                        //20151028 - gerry added new audit log
                        AuditLog auditLog = new AuditLog(haulierJobTrip.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Trip", haulierJobTrip.Sequence, "EDT");
                        auditLog.WriteAuditLog("SeqNo", haulierJobTrip.Sequence.ToString(), "", con, tran);
        ...
        public bool DeleteHaulierJobTrips(List<HaulierJobTrip> haulierJobTrips, string frmName, string user)
        {
            ...
            try
            {
                foreach (HaulierJobTrip trip in haulierJobTrips)
                {
                    ...
                    {
                        if (!ValidateDeletingHaulierJobTrip(trip, trip.LegType == Leg_Type.OneLeg ? 1 : 2, quotationNo))
                        {
                        ...
                        if (trip.isMulti_leg && trip.LegGroupMember <= 2) // && !trip.ContainerNo.Equals(string.Empty))
                        {
                            ...
                            foreach (HaulierJobTrip tempJobTrip in tripListSameLegGroup)
                            {
                                ...
                                if (tempJobTrip.TripStatus != JobTripStatus.Completed && tempJobTrip.Sequence != trip.Sequence && tempJobTrip.LegGroupMember <= 2)
                                {
                                    ...
                                    tempJobTrip.LegType = Leg_Type.OneLeg;
                                    tempJobTrip.PartnerLeg = 0;
                                    //Do edit
                                    HaulierJobDAL.EditHaulierJobTrip(tempJobTrip, con, tran);
                                    //20151028 - gerry added new audit log
                                    AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Trip", tempJobTrip.Sequence, "EDT");
                                    auditLog.WriteAuditLog("SeqNo", tempJobTrip.Sequence.ToString(), string.Empty, con, tran);
                        ...
                        if (HaulierJobDAL.DeleteHaulierJobTrip(trip, con, tran))
                        {
                            ...
                            // 2014-08-20 Zhou Kai ends
                            //20151028 - gerry added new audit log
                            AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Trip", trip.Sequence, "DEL");
                        ...
                        {
                            HaulierJob hubbingJob = HaulierJob.GetHaulierJob(trip.PartnerJobNo, con, tran);
                            List<string> contNoList = new List<string>();
                            contNoList.Add(trip.ContainerNo);
                            HaulierJob.UpdatePartnerJobNos(hubbingJob.JobID, contNoList, string.Empty, con, tran);
                        }
                    }
                }
                ...
                foreach (HaulierJobTrip trip in haulierJobTrips)
                {
                    ...
                    if (trip.ValidateEditHaulierJobTrip())
                    {
                        ...
                        if (trip.JobTripStates[trip.JobTripStates.Count - 1].Status != trip.TripStatus)
                        {
                            ...
                            // Create a JobTripState and write to DB
                            JobTripState jobTripState = new JobTripState(1, trip.TripStatus, this.JobDateTime, " new state" + user, true);
                            trip.AddJobTripState(jobTripState, con, tran);
                        }
                        //DoAddOrEditLogging(trip, frmName, user, FormMode.Edit, con, tran);

                        //20151028 - gerry added new audit log
                        AuditLog auditLog = new AuditLog(trip.JobNo, "HL", "BK", trip.JobID, user, DateTime.Now, "Job Trip", trip.Sequence, "EDT");
        ...
        public bool DeleteHaulierJobTrip(HaulierJobTrip haulierJobTrip, string frmName, string user)
        {
            ...
            try
            {
                if (haulierJobTrip.ValidateDeleteHaulierJobTrip())
                {
                    ...
                    if (!ValidateDeletingHaulierJobTrip(haulierJobTrip, haulierJobTrip.LegType == Leg_Type.OneLeg ? 1 : 2, quotationNo))
                    {
                        throw new FMException(TptResourceBLL.ErrNo1LegRate);
                    }
                    //for more than 2 legs dont allow to delete the first or 2nd legs
                    ...
                    if (HaulierJobDAL.DeleteHaulierJobTrip(haulierJobTrip, con, tran))
                    {
                        ...
                        // 2014-08-20 Zhou Kai ends
                        //20151028 - gerry added new audit log
                        AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Trip", haulierJobTrip.Sequence, "DEL");
            ...
            if (haulierJobCharge.ValidateAddHaulierJobCharge(this, haulierJobCharge))
            {
                ...
                try
                {
                    if (HaulierJobDAL.AddHaulierJobCharges(haulierJobCharge, con, tran) == true)
                    {
                        ...
                        ////20151028 - gerry added new audit log
                        AuditLog auditLog = new AuditLog(haulierJobCharge.ChargeCode, "HL", frmName.ToString(), this.JobID, user, DateTime.Now, "Job Charge", haulierJobCharge.SequenceNo, "ADD");
                        auditLog.WriteAuditLog("SeqNo", haulierJobCharge.SequenceNo.ToString(), string.Empty, con, tran);
            ...
            if (haulierJobCharge.ValidateAddHaulierJobCharge(haulierJob, haulierJobCharge))
            {
                ...
                try
                {
                    if (HaulierJobDAL.AddHaulierJobCharges(haulierJobCharge, con, tran) == true)
                    {
                        ...
                        ////20151028 - gerry added new audit log
                        AuditLog auditLog = new AuditLog(haulierJob.JobNo, "HL", "BK", haulierJob.JobID, user, DateTime.Now, "Job Charge", haulierJobCharge.SequenceNo, "ADD");
                        auditLog.WriteAuditLog("SeqNo", haulierJobCharge.SequenceNo.ToString(), string.Empty, con, tran);
        ...
        public bool EditHaulierJobCharges(HaulierJobCharge haulierJobCharge, string frmName, string user)
        {
            if (haulierJobCharge.ValidateEditJobCharge() == true)
            {
                ...
                try
                {
                    HaulierJobCharge oldHaulierJobTrip = HaulierJobDAL.GetHaulierJobCharge(haulierJobCharge.JobID, haulierJobCharge.SequenceNo, con, tran);
                    if (HaulierJobDAL.EditHaulierJobCharges(haulierJobCharge, con, tran) == true)
                    {
                        ...
                        //20151028 - gerry added new audit log
                        AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Charge", haulierJobCharge.SequenceNo, "EDT");
            ...
            if (haulierJobCharge.ValidateDeleteJobCharge() == true)
            {
                ...
                try
                {
                    if (HaulierJobDAL.DeleteHaulierJobCharges(haulierJobCharge, con, tran) == true)
                    {
                        ...
                        //20151028 - gerry added new audit log
                        AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, frmName, haulierJobCharge.SequenceNo, "DEL");
                        auditLog.WriteAuditLog(this, null, con, tran);
        ...
        public bool SetHaulierJobToComplete(string formName, string user)
        {
            ...
            {
                string outMsg = "";
                this.JobTrips = HaulierJobTrip.GetAllHaulierJobTrips(this);
                if (IsHaulierCanSetToComplete(out outMsg))
                {
                    ...
                    this.LastModified_By = user;
                    this.LastModifiedDateTime = DateTime.Now;
                    HaulierJobDAL.SetHaulierJobStatus(this.JobID, this.JobStatus, this.remarks5, user, con, tran);

                    Dictionary<string, Decimal> dictContainerQty = GetContainerQtyByJobID_New(this.JobID, con, tran);
                    TransportFacadeOut.EditTptJobToSpecialJobDetail(this, con, tran, HaulierJobDAL.GetPrefix(), dictContainerQty);

                    foreach(HaulierJobTrip jt in this.JobTrips)
                    {
                        jt.TripStatus = JobTripStatus.Completed;
                        HaulierJobDAL.EditHaulierJobTrip(jt, con, tran);
                    }
                    #region OLD logging removed
                    //// now form the log entry
                    //DateTime serverDateTime = Logger.GetServerDateTime();
                    ...
                    //20151028 - gerry added new audit log
                    AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, formName, 0, "EDT");
        ...
        private bool IsHaulierCanSetToComplete(out string errMsg)
        {
            ...
            {
                if (tempJobTrip.TripStatus != JobTripStatus.Completed && tempJobTrip.isMovementJob)
                {
            ...
            try
            {
                ...
                if (ValidateAddHaulierJobHeader())   //job trip validation was transfered in the job trip loop
                {
                    ...
                    if (!existingJob)
                    {
                        ...
                        {
                            Dictionary<string, Decimal> dictContainerQty = GetContainerQtyByJobID_New(this.JobID, con, tran);
                            TransportFacadeOut.AddTptJobToSpecialJobDetail(this, con, tran, prefix, dictContainerQty);
                        }
                        //20140909 end
                        ...
                        //20151028 - gerry added new audit log for Job header
                        AuditLog auditLog = new AuditLog(this.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Header", 0, "ADD");
                        auditLog.WriteAuditLog("JobNumber", this.JobNo, string.Empty, con, tran);
                    }
                    ...
                    {
                        //20160603 - gerry replaced code below
                        List<string> conNos = new List<string>();
                        foreach (HaulierJobTrip t in this.JobTrips)
                        {
                            HaulierJob outJob = HaulierJob.GetHaulierJob(t.PartnerJobNo, con, tran);
                            if(t.PartnerJobNo != string.Empty)
                                conNos.Add(t.ContainerNo);
                            HaulierJob.UpdatePartnerJobNos(outJob.JobID, conNos, this.JobNo, con, tran);
                            //20151028 - gerry added new audit log for partnerJob for each job trip
                            AuditLog auditLog = new AuditLog(t.JobNo, "HL", "BK", t.JobID, user, DateTime.Now, "Job Trip", t.Sequence, "EDT");
                            auditLog.WriteAuditLog("Hubbing JobNumber", this.JobNo, string.Empty, con, tran);
                        }
                    }
                    ...
                    {   //assign job header values to job trip
                        ...
                        if (jobTrip.LegGroup == -1)
                        {
                            if (jobTrip.ContainerNo == String.Empty)
                                jobTrip.LegGroup = HaulierJobDAL.GetNextLegGroup(jobTrip.JobID, con, tran);
                            else
                                jobTrip.LegGroup = HaulierJobDAL.GetNextLegGroupByContainerNo(jobTrip.JobID, jobTrip.ContainerNo, con, tran);
                        }
                        if (jobTrip.LegGroupMember == -1)
                        {
                            jobTrip.LegGroupMember = HaulierJobDAL.GetNextLegGroupMember(jobTrip.JobID, jobTrip.LegGroup, con, tran);
                        }
                        if (option.setting_Value.ToString() == "T")
                            jobTrip.TripStatus = jobTrip.LegGroupMember == 1 ? JobTripStatus.Ready : jobTrip.TripStatus;//set to ready
                        //20170112 ends
                            ...
                            if (jobTrip.Sequence == 0 || !bPlanningOrSeaFreightSide)
                            {
                                ...
                                if (!this.quotationNo.Equals(string.Empty) && jobTrip.isBillable)
                                {
                                    ...
                                    HashSet<string> rateUoms =
                                        HaulierJobTrip.GetHaulierUOMFromQuotation(jobTrip.CustomerCode, isStopDependent,this.jobDateTime, jobTrip.LegType);
                                    if (rateUoms.Count > 0)
                                    {
                                ...
                                //JobTripState jobTripBookedState = new JobTripState(1, JobTripStatus.Booked, this.JobDateTime,
                                //    (bPlanningOrSeaFreightSide ? TptResourceUI.AddFromPlanning : TptResourceUI.ImportedFromSeaFreight) + " by " + user, true);
                                JobTripState jobTripBookedState = new JobTripState(1, jobTrip.TripStatus, this.JobDateTime,
                                    (bPlanningOrSeaFreightSide ? TptResourceUI.AddFromPlanning : TptResourceUI.ImportedFromSeaFreight) + " by " + user, true);
                                ...
                                if (jobTrip.LegType == Leg_Type.FirstOfTwoLeg ||
                                        (
                                    ...
                                    )
                                {
                                    ...
                                    if (!bPlanningOrSeaFreightSide && isReadyAfterTransfer)
                                    {
                                        ...
                                        JobTripState jobTripReadyState = new JobTripState(2, JobTripStatus.Ready, this.JobDateTime,
                                ...
                                if (bPlanningOrSeaFreightSide)
                                {
                                    ...
                                    JobTripState jobTripAssignedState = new JobTripState(3, JobTripStatus.Ready, this.JobDateTime,
                                       (bPlanningOrSeaFreightSide ? TptResourceUI.AddFromPlanning : TptResourceUI.ImportedFromSeaFreight) + " by "+user, true);
                                ...
                                ////20151028 - gerry added new audit log
                                AuditLog auditLog = new AuditLog(jobTrip.JobNo, "HL", "BK", this.JobID, user, DateTime.Now, "Job Trip", jobTrip.Sequence, "ADD");
                                auditLog.WriteAuditLog("SeqNo", jobTrip.Sequence.ToString(), string.Empty, con, tran);
                            }
                        }
                    }
        ...
        {
            return HaulierJobDAL.GetContainerQtyByJobID_New(jobId, con, tran);
        }
  ...
  " ConCode:" + t.ContainerCode + " ConNO:" + t.ContainerNo + " PartnerJNO: " + t.PartnerJobNo);
                    ...
                    try // 2014-10-20 Zhou Kai adds try catch
                    {
                        transportRate =
                           quotation.TransportRates.First(x => x.UOM.ToUpper().Equals(uom) &&
                               ...
                               x.NoOfLeg == (t.LegType == Leg_Type.OneLeg ? 1 : 2));
                    }
                    ...
                    if (t.isMulti_leg && (t.LegType == Leg_Type.FirstOfTwoLeg))
                    {
            ...
            (HaulierJob haulierJob, TransportRate transportRate, HaulierJobTrip haulierJobTrip)
        {
            try
            {
                ...
                haulierJobCharge.ExchangeRate = (CurrencyRate.GetCurrencyRate(transportRate.Currency, haulierJob.jobDateTime)).Rate;
                haulierJobCharge.JobChargeType = "T";
                Gst tempGst = Gst.GetGstRate(charge.TaxCode, haulierJob.JobDateTime);
        ...
        {
            if (haulierJob.JobStatus == JobStatus.Completed) { throw new FMException(TptResourceUI.CantModifyCompleteJob); }
                                                         ...
                                                         SqlTransaction dbTran)
        {
            try
            {
                ...
                foreach (HaulierJobTrip t in newTrips) { allTrips.Add(t); }
                ...
                HaulierJobDAL.AddHaulierJobTrips(job, newTrips, formName, usrId, dbCon, dbTran);
                //20170726
        ...
        public static List<HaulierJobTrip> CreateSampleTripsForSeaJob_old(SeaJob sJob, HaulierJob inJob, /*HaulierJob outJob,*/ List<Stop> stops)
        {
                ...
                for (int i = 0; i < stops.Count - 1; i++)
                {
                    ...
                    foreach (IGrouping<string, SeaJobContainer> g in containerGrps)
                    {
                        HaulierJobTrip t = new HaulierJobTrip()
                        {
                            ...
                            isBillable = true,
                            isMulti_leg = true, //stops.Count > 2 ? true : false,
                            IsNew = true,
                            IsLaden = isLaden,
                            StartStop =(Stop)stops[i].Clone(), // outJob == null ? (Stop)stops[i].Clone() : (i == 0 ? lastStopOfHubbingJob : (Stop)stops[i].Clone()),
                        ...
                        if (isSeaExport && i == 0) { t.IsLaden = false; }
        ...
        //20160705 - gerry created new method to create sample trip
        public static List<HaulierJobTrip> CreateSampleTripsForSeaJobFreshContainers(SeaJob sJob, HaulierJob inJob, /*HaulierJob outJob,*/ List<Stop> stops)
        {
            List<HaulierJobTrip> sampleTrips = new List<HaulierJobTrip>();
            try
            {
                OperationDetail od = sJob is SeaExportJob ?
                                        OperationDetail.GetOperationDetail(sJob.ShipperCode) :
                                        OperationDetail.GetOperationDetail(sJob.ConsigneeCode);
                CustomerDTO cud = (od == null || od.customerCode == String.Empty) ?
                                        CustomerDTO.GetCustomerByCustCode("CASH") :
                ...
                for (int i = 0; i < stops.Count - 1; i++)
                {
                    bool isLaden = sJob is SeaExportJob ? (i == 0 ? false : true) : (i == stops.Count - 2 ? false : true);
                    ...
                    foreach (IGrouping<string, SeaJobContainer> g in containerGrps)
                    {
                        ...
                        // default the first leg of sea export to not laden
                        // 2014-11-20 Zhou Kai adds condition: outJob == null
                        if (sJob is SeaExportJob && i == 0) { t.IsLaden = false; }
                        // cannot default the second leg of sea import to not laden
                        // if (!isSeaExport && i == 1) { t.IsLaden = false; }            
        ...
        //20160718 - gerry created new method to create sample trip for hubbing container
        public static List<HaulierJobTrip> CreateSampleTripsForSeaJobHubbingContainers(SeaJob sJob, List<string> selectedHubbingJobNos, List<Stop> stops)
        {
            List<HaulierJobTrip> sampleTrips = new List<HaulierJobTrip>();
            try
            {
                foreach (string hubbingJobNo in selectedHubbingJobNos)
                {
                    List<Stop> tempStopList = new List<Stop>();
                    foreach (Stop stop in stops) { tempStopList.Add(stop); }
                    //insert last stop of hubbinh container
                    tempStopList.Insert(0, HaulierJob.GetLastStopOfHubbingJob(sJob, hubbingJobNo));
                    //get Haulier Job object based on hubbing jobNo
                    HaulierJob hubbingJob = HaulierJob.GetHaulierJob(hubbingJobNo);
                    CustomerDTO customer = CustomerDTO.GetCustomerByCustCode(hubbingJob.CustNo);
                    for (int i = 0; i < tempStopList.Count - 1; i++)
                    {
                        HaulierJobTrip t = new HaulierJobTrip()
                        {
                            JobID = hubbingJob.JobID,
                            Sequence = i,
                            JobNo = hubbingJob.JobNo,
                            isBillable = true,
                            isMulti_leg = true,
                            IsLaden = true,
                            StartStop = (Stop)tempStopList[i].Clone(),
                            EndStop = (Stop)tempStopList[i + 1].Clone(),
                            TripStatus = JobTripStatus.Booked,
                            CustomerCode = customer.Code,
                            CustomerName = customer.Code,
                            jobType = hubbingJob.jobType,
                        ...
                        if (t.StartStop.Code.Equals(t.EndStop.Code))
                            throw new FMException(TptResourceBLL.ErrSameStop+"("+t.StartStop.Code.ToString() +")");
                        if (!sampleTrips.Contains(t))
                            sampleTrips.Add(t);
                    }
                }
        ...
        /// <returns></returns>
        public static SortableList<HaulierJobTrip> CreateConcreteTripsFromSampleTripsForStandaloneJob( HaulierJob job, Dictionary<string, int> containerCodeToQty, List<HaulierJobTrip> sampleTrips)
        {
            SortableList<HaulierJobTrip> allTrips = new SortableList<HaulierJobTrip>();
        ...
        public static SortableList<HaulierJobTrip> CreateConcreteTripsFromSampleTripsForSeaFreight_old(
            ...
            Dictionary<string, int> dicConNoToLegGroup)
        {
            ...
            try
            {
                ...
                {
                    int nLegGroup, nLegGroupMember = 1;
                    if (outJob != null)
                    {
                        ...
                        {
                            nLegGroup = allTrips[allTrips.Count - 1].LegGroup + 1;
                            nLegGroupMember = 1;
                        }
                    }
                    else
                    ...
                    if (outJob != null)
                    {
                        ...
                        else //20160506 - create partner leg for fresh container
                        {
                            HaulierJobTrip firstLegTrip = null;
                            foreach (HaulierJobTrip sampleTrip in sampleTrips)
                            {
                                if (sc.ContainerSize == sampleTrip.DGRemarks)
                                {
                                    firstLegTrip = sampleTrip.Clone() as HaulierJobTrip;
                                    ...
                                    firstLegTrip.PartnerLeg = 2;
                                    firstLegTrip.Sequence = 1;
                                    firstLegTrip.isMulti_leg = true;
                                    //firstLegTrip
                                    firstLegTrip.DGRemarks = sc.ContainerSize;
                                    //modify the sample jobtrip
                                    sampleTrip.PartnerLeg = firstLegTrip.Sequence;
                                    sampleTrip.StartStop = Stop.GetStop(firstLegTrip.EndStop.Code);
                                    sampleTrip.isMulti_leg = true;
                                }
                            }
                            if (firstLegTrip != null)
                    ...
                    foreach (HaulierJobTrip t in sampleTrips)
                    {
                        ...
                        {                            
                            ...
                            newTrip.GrossWeight = sc.GrossWeight; // t.GrossWeight;
                            newTrip.GrossCBM = sc.GrossCBM == 0 ? container.TareCBM : sc.GrossCBM;
                            //ContainerTypesDTO ct = ContainerTypesDTO.GetContainerDTO(sc.ContainerCode);
                            newTrip.maxWeight = sc.GrossWeight;
                            newTrip.maxCBM = sc.GrossCBM == 0 ? container.TareCBM : sc.GrossCBM;
        ...
        public static Stop GetLastStopOfHubbingJob(SeaJob sJob, string hubbingJobNo)
        {
            try
            {
                HaulierJob hubbingJob = GetHaulierJob(hubbingJobNo);
                foreach (SeaJobContainer sc in sJob.SeaJobContainers)
                {
                    if (sc.ContainerNumber != string.Empty)
                    {
                        //HaulierJob hubbingJob = GetHubbingJob(sJob.CustomerCode, sc.ContainerNumber);
                        if (hubbingJob != null)
                        {
                            var jobTrips = HaulierJobTrip.GetAllHaulierJobTrips(hubbingJob).Where(jt => jt.ContainerNo == sc.ContainerNumber).OrderByDescending(jt => jt.LegGroupMember).ToList();
                            if (jobTrips.Count > 0)
                                return jobTrips[0].EndStop;
                        }
                    }
                }
            }
        ...
        //20160601 - gerry created new method to cater multiple hubbing jobs
        public static SortableList<HaulierJobTrip> CreateConcreteTripsFromSampleTripsForSeaFreight_1(HaulierJob hJob, SeaJob sJob, List<HaulierJobTrip> sampleTrips, 
            out bool hasHubbingJob, out List<string> hubbingJobContainerNos)
        {
            SortableList<HaulierJobTrip> allTrips = new SortableList<HaulierJobTrip>();
            try
            {
                hasHubbingJob = false;
                hubbingJobContainerNos = new List<string>();
                int nConIndex = 1;
                int nLegGroup = 0;
                foreach (SeaJobContainer sc in sJob.SeaJobContainers)
                {
                    List<HaulierJobTrip> tempSampleTrips = new List<HaulierJobTrip>();
                    foreach (HaulierJobTrip trip in sampleTrips) { tempSampleTrips.Add(trip); }
                    HaulierJob hubbingJob = null;
                    HaulierJobTrip firstLegTrip = null;
                    nLegGroup++;
                    int nLegGroupMember = 0;
                    int nSeq = allTrips.Count;
                    ContainerTypesDTO container = ContainerTypesDTO.GetContainerDTO(sc.ContainerCode);
                    if (sc.ContainerNumber != string.Empty)
                        hubbingJob = GetHubbingJob(sJob.CustomerCode, sc.ContainerNumber);

                    if (hubbingJob != null)
                    {
                        hasHubbingJob = true;
                        hubbingJobContainerNos.Add(sc.ContainerNumber);
                        //if (!hasHubbingJob) { throw new FMException("Unable to transfer job because there is a hubbing detected."); }
                        hubbingJob.JobTrips = HaulierJobTrip.GetAllHaulierJobTrips(hubbingJob);
                        foreach (HaulierJobTrip trip in hubbingJob.jobTrips)
                        {
                            if (trip.ContainerNo == sc.ContainerNumber)
                                hJob.PartnerJobTrips.Add((HaulierJobTrip)trip.Clone());
                        }

                        nLegGroupMember = GetJobTripsInTheSameContainerNo(hubbingJob.JobTrips, sc.ContainerNumber).Count();
                        //if there is additional leg
                        var tempTripWithTheContNo = allTrips.Where(jt => jt.ContainerNo == sc.ContainerNumber).ToList();
                        if (tempTripWithTheContNo.Count > 0)
                            nLegGroupMember = tempTripWithTheContNo.Max(jt => jt.LegGroupMember);
                    ...
                    else
                    {
                        ...
                        if (hasHubbingJob)
                        {
                            foreach (HaulierJobTrip freshJobTrip in sampleTrips)
                            {
                                if (freshJobTrip.DGRemarks == sc.ContainerSize)
                                {
                                    nLegGroupMember = 1;
                                    firstLegTrip = freshJobTrip.Clone() as HaulierJobTrip;
                                    firstLegTrip.JobID = hJob.JobID;
                                    firstLegTrip.JobNo = hJob.JobNo;
                                    firstLegTrip.Sequence = nSeq;
                                    firstLegTrip.LegGroup = nLegGroup;
                                    firstLegTrip.SetLegGroupMember(1);
                                    firstLegTrip.PartnerJobNo = string.Empty;
                                    firstLegTrip.ContainerNo = sc.ContainerNumber;
                                    firstLegTrip.IsLaden = false;
                                    firstLegTrip.GrossWeight = 0;
                                    firstLegTrip.GrossCBM = 0;
                                    firstLegTrip.StartStop = Stop.GetStop(sJob.PickUpPlaceCode);
                                    firstLegTrip.EndStop = Stop.GetStop(sJob.StuffingCode);
                                    firstLegTrip.PartnerLeg = firstLegTrip.Sequence + 1;
                                    firstLegTrip.LegType = Leg_Type.FirstOfTwoLeg;
                                    firstLegTrip.isMulti_leg = true;
                                    firstLegTrip.JobTripStates = new SortableList<JobTripState>();
                                    firstLegTrip.TripStatus = JobTripStatus.Ready;
                                    firstLegTrip.DGRemarks = sc.ContainerSize;
                                    nLegGroupMember++;
                                    //modify the sample jobtrip
                                    freshJobTrip.PartnerLeg = firstLegTrip.Sequence;
                                    freshJobTrip.StartStop = Stop.GetStop(firstLegTrip.EndStop.Code);
                                    freshJobTrip.SetLegGroupMember(nLegGroupMember);
                                    freshJobTrip.isMulti_leg = true;
                                    firstLegTrip.TripStatus = JobTripStatus.Booked;

                                    tempSampleTrips.Insert(0, firstLegTrip);
                                    break;
                                }
                            }
                        }
                    }
                    foreach (HaulierJobTrip sampleJobTrip in tempSampleTrips)
                    {
                        if (sampleJobTrip.DGRemarks == sc.ContainerSize)
                        {
                            //now add the sample job trip
                            HaulierJobTrip newTrip = (HaulierJobTrip)sampleJobTrip.Clone();
                            newTrip.JobID = hJob.JobID;
                            newTrip.JobNo = hJob.JobNo;
                            newTrip.Sequence = nSeq++;
                            newTrip.LegGroup = nLegGroup;
                            if (hubbingJob != null) { newTrip.SetLegGroupMember(nLegGroupMember); }
                            else if (!hasHubbingJob && hubbingJob == null)
                            {
                                nLegGroupMember++;
                                newTrip.SetLegGroupMember(nLegGroupMember);
                            }
                            newTrip.LegType = firstLegTrip != null ? Leg_Type.SecondOfTwoLeg : Leg_Type.OneLeg;
                            newTrip.PartnerLeg = firstLegTrip != null ? firstLegTrip.Sequence : 0;
                            newTrip.ContainerNo = sc.ContainerNumber;
                            newTrip.ContainerCode = sc.ContainerCode;
                            newTrip.SealNo = sc.SealNumber;
                            //newTrip.DGRemarks = String.Empty;
                            newTrip.subCon = new OperatorDTO(sampleJobTrip.subCon.Code, sampleJobTrip.subCon.Name, sampleJobTrip.subCon.Add1, sampleJobTrip.subCon.Add2, sampleJobTrip.subCon.Add3, sampleJobTrip.subCon.Add4, sampleJobTrip.subCon.City);
                            newTrip.OwnTransport = sampleJobTrip.subCon.Code == String.Empty;
                            newTrip.PartnerJobNo = hubbingJob == null ? String.Empty : hubbingJob.JobNo;
                            newTrip.GrossWeight = sc.GrossWeight; // t.GrossWeight;
                            newTrip.GrossCBM = sc.GrossCBM == 0 ? container.TareCBM : sc.GrossCBM;
                            newTrip.maxWeight = sc.GrossWeight;
                            newTrip.maxCBM = sc.GrossCBM == 0 ? container.TareCBM : sc.GrossCBM;
                            newTrip.JobTripStates = new SortableList<JobTripState>();

                            if (newTrip.LegGroupMember == 1 || newTrip.PartnerJobNo != string.Empty)
                                newTrip.TripStatus = JobTripStatus.Ready;
                            allTrips.Add(newTrip);
                        }
                    }
                    //if (outJob == null && firstLegTrip != null & hasHubbingJob)
                    //{
                    //    allTrips.Add(firstLegTrip);
                    //}
                }
            }
            catch (FMException ex) { throw ex; }
        ...
        //20160705 - gerry created new method to cater multiple hubbing jobs mixed with fresh collect containers
        public static SortableList<HaulierJobTrip> CreateConcreteTripsFromSampleTripsForSeaFreightMultipleHubbingJob(HaulierJob hJob, SeaJob sJob, List<HaulierJobTrip> sampleTripFreshContainers,
            List<HaulierJobTrip> sampleTripsHubbingContainers, out bool hasHubbingJob, out List<string> hubbingJobContainerNos)
        {
            SortableList<HaulierJobTrip> allTrips = new SortableList<HaulierJobTrip>();
            try
            {
                ...
                foreach (SeaJobContainer sc in sJob.SeaJobContainers)
                {
                    List<HaulierJobTrip> tempSampleTripFreshContainers = new List<HaulierJobTrip>();
                    foreach (HaulierJobTrip trip in sampleTripFreshContainers) { tempSampleTripFreshContainers.Add(trip); }
                    HaulierJob hubbingJob = null;
                    HaulierJobTrip firstLegTrip = null;
                    nLegGroup++;
                    int nLegGroupMember = 0;
                    ...
                    if (hubbingJob != null)
                    {
                        ...
                        ////for hubbing job always take the last stop captured during creation of sample trip
                        ////additional leg for hubbing job
                        //HaulierJobTrip jtAdditionalLeg = tempSampleTrips[tempSampleTrips.Count - 1];
                        foreach (HaulierJobTrip tempHubbingJobTrip in sampleTripsHubbingContainers)
                        {
                            if (tempHubbingJobTrip.JobNo == hubbingJob.JobNo)
                            {
                                //if there is additional leg
                                var tempTripWithTheSameContNo = allTrips.Where(jt => jt.ContainerNo == sc.ContainerNumber).ToList();
                                if (tempTripWithTheSameContNo.Count > 0)
                                    nLegGroupMember = tempTripWithTheSameContNo.Max(jt => jt.LegGroupMember);
                                nLegGroupMember++;
                                #region Create additional leg for hubbing job
                                HaulierJobTrip newTrip = (HaulierJobTrip)tempHubbingJobTrip.Clone();
                                newTrip.JobID = hJob.JobID;
                                newTrip.JobNo = hJob.JobNo;
                                newTrip.Sequence = nSeq++;
                                //start and end stop assigned in sample trip and was cloned()
                                //newTrip.StartStop = hubbingJob.JobTrips.Where(jt => jt.ContainerNo == sc.ContainerNumber).OrderByDescending(jt => jt.LegGroupMember).ToList()[0].EndStop;
                                newTrip.LegGroup = nLegGroup;
                                newTrip.SetLegGroupMember(nLegGroupMember);
                                newTrip.LegType = Leg_Type.OneLeg;
                                newTrip.PartnerJobNo = hubbingJob.JobNo;
                                newTrip.ContainerNo = sc.ContainerNumber;
                                newTrip.ContainerCode = sc.ContainerCode;
                                newTrip.SealNo = sc.SealNumber;
                                //newTrip.DGRemarks = String.Empty;
                                newTrip.subCon = new OperatorDTO(tempHubbingJobTrip.subCon.Code, tempHubbingJobTrip.subCon.Name, tempHubbingJobTrip.subCon.Add1, tempHubbingJobTrip.subCon.Add2, tempHubbingJobTrip.subCon.Add3, tempHubbingJobTrip.subCon.Add4, tempHubbingJobTrip.subCon.City);
                                newTrip.OwnTransport = tempHubbingJobTrip.subCon.Code == String.Empty;
                                newTrip.GrossWeight = sc.GrossWeight; // t.GrossWeight;
                                newTrip.GrossCBM = sc.GrossCBM == 0 ? container.TareCBM : sc.GrossCBM;
                                newTrip.maxWeight = sc.GrossWeight;
                                newTrip.maxCBM = sc.GrossCBM == 0 ? container.TareCBM : sc.GrossCBM;
                    ...
                    {
                        foreach (HaulierJobTrip sampleJobTrip in tempSampleTripFreshContainers)
                        {
                            if (sampleJobTrip.DGRemarks == sc.ContainerSize)
                            {
                                //now add the sample job trip
                                #region Create job trips for fresh containers
                                HaulierJobTrip newTrip = (HaulierJobTrip)sampleJobTrip.Clone();
                                newTrip.JobID = hJob.JobID;
                                newTrip.JobNo = hJob.JobNo;
                                newTrip.Sequence = nSeq++;
        ...
        //20160705 - gerry created new method to cater multiple hubbing jobs mixed with fresh collect containers
        public static SortableList<HaulierJobTrip> CreateConcreteTripsFromSampleTripsForSeaFreight(HaulierJob hJob, SeaJob sJob, List<HaulierJobTrip> sampleTrips,
            out bool hasHubbingJob, out List<string> hubbingJobContainerNos)
        {
            SortableList<HaulierJobTrip> allTrips = new SortableList<HaulierJobTrip>();
            try
            {
                ...
                foreach (SeaJobContainer sc in sJob.SeaJobContainers)
                {
                    ...
                    if (hubbingJob != null)
                    {
                        ...
                        #region Create additional leg for hubbing job
                        HaulierJobTrip newTrip = (HaulierJobTrip)jtAdditionalLeg.Clone();
                        newTrip.JobID = hJob.JobID;
                        newTrip.JobNo = hJob.JobNo;
                        newTrip.Sequence = nSeq++;
                        newTrip.StartStop = hubbingJob.JobTrips.Where(jt => jt.ContainerNo == sc.ContainerNumber).OrderByDescending(jt => jt.LegGroupMember).ToList()[0].EndStop;
                        ...
                        newTrip.SealNo = sc.SealNumber;
                        //newTrip.DGRemarks = String.Empty;
                        newTrip.subCon = new OperatorDTO(jtAdditionalLeg.subCon.Code, jtAdditionalLeg.subCon.Name, jtAdditionalLeg.subCon.Add1, jtAdditionalLeg.subCon.Add2, jtAdditionalLeg.subCon.Add3, jtAdditionalLeg.subCon.Add4, jtAdditionalLeg.subCon.City);
                        newTrip.OwnTransport = jtAdditionalLeg.subCon.Code == String.Empty;
                        newTrip.GrossWeight = sc.GrossWeight; // t.GrossWeight;
                        newTrip.GrossCBM = sc.GrossCBM == 0 ? container.TareCBM : sc.GrossCBM;
                        newTrip.maxWeight = sc.GrossWeight;
                        newTrip.maxCBM = sc.GrossCBM == 0 ? container.TareCBM : sc.GrossCBM;
                    ...
                    if (kvp.Key != String.Empty && kvp.Value > 0)
                    {
                        ...
                        for (int i = 0; i < stopChian.Count - 1; i++)
                        {
                            sampleTrips.Add(new HaulierJobTrip()
                            {
                                ...
                                OwnTransport = true,
                                LegType = stopChian.Count == 2 ? Leg_Type.OneLeg :
                                   (i == 0 ? Leg_Type.FirstOfTwoLeg : (i == 1 ? Leg_Type.SecondOfTwoLeg :
                                    /*LegType.MoreThanTwoLegs*/ Leg_Type.OneLeg)),
        ...
        public static List<Stop> GetPreDefinedStopsFromSeaFreightSide(SeaJob sJob,  bool bHasTransferOutJob)
        {
            ...
            try
            {
                var freshCollectContainers = sJob.SeaJobContainers.Where(sc => sc.ContainerNumber == "").ToList();
                bHasTransferOutJob = !(freshCollectContainers.Count > 0); //
        ...
        public bool TransferHubbingJobTripsToNewJob_old(HaulierJob hubbingJob, List<string> selectedContainerNos, List<Stop> additionalStops, SortableList<HaulierJobTrip> sampleTrips, string frmName, string userID, string quotationNo, bool isBillByTransport)
        {
            SqlConnection con = new SqlConnection(FMGlobalSettings.TheInstance.getConnectionString());
            SqlTransaction tran = null;
            try
            {
                if (selectedContainerNos.Count > 0 && additionalStops.Count > 0)
                {
                    ...
                    newHaulerJob.CustNo = hubbingJob.custNo;
                    ...
                    newHaulerJob.BranchCode = hubbingJob.BranchCode;
                    newHaulerJob.TptDeptCode = hubbingJob.TptDeptCode;
                    ...
                    newHaulerJob.haulierChargeType = hubbingJob.haulierChargeType;
                    newHaulerJob.jobType = hubbingJob.jobType;
                    newHaulerJob.IsBillable = true;
                    newHaulerJob.jobTransferType = JobTransferType.TransferIn;
                    newHaulerJob.PartnerJobNo = hubbingJob.JobNo;
                    */
                    #endregion
                    ...
                    foreach (HaulierJobTrip trip in hubbingJob.jobTrips)
                    {
                        foreach (string contNo in selectedContainerNos)
                        {
                            ...
                            {
                                this.PartnerJobTrips.Add((HaulierJobTrip)trip.Clone());
                                break;
                            }
                        }
                    }
                    //validate HaulierJob
                    if (this.ValidateAddHaulierJobHeader())   //job trip validation was transfered in the job trip loop
                    {
                        if (con.State == ConnectionState.Closed) { con.Open(); }
                        if (tran == null) { tran = con.BeginTransaction(); }
                        ...
                        string prefix = HaulierJobDAL.GetPrefix();
                        //HaulierJobDAL.AddHaulierHeader(this, con, tran, prefix, userID); // no need to add
                        if (this.BillByTransport && this.IsBillable)
                        {
                            Dictionary<string, Decimal> dictContainerQty = GetContainerQtyByJobID_New(this.JobID, con, tran);//20170726
                            TransportFacadeOut.AddTptJobToSpecialJobDetail(this, con, tran, prefix, dictContainerQty);
                        ...
                        int maxLegGroup = 0;
                        if (this.JobTrips.Count > 0)
                            maxLegGroup = this.JobTrips.AsEnumerable().Max(jt => jt.LegGroup);

                        //foreach (HaulierJobTrip t in this.PartnerJobTrips)
                        //{ t.PartnerJobNo = this.JobNo; }
                        // 2014-10-31 Zhou Kai ends
                        //
                        //update parnerJobNo of the hubbingJob
                        HaulierJobDAL.UpdatePartnerTrips(hubbingJob.JobID, selectedContainerNos, this.JobNo, con, tran);
                        ...
                        {
                            HaulierJobTrip newLegTrip = null;
                            foreach (Stop stop in additionalStops)
                            {  //get the list of job trips based on conatinerNo or the legGroup(I used containerNo)
                                SortableList<HaulierJobTrip> tripsInSameContainerNo = GetJobTripsInTheSameContainerNo(this.PartnerJobTrips, containerNo);
                                //create haulier job trip
                                newLegTrip = (HaulierJobTrip)tripsInSameContainerNo[0].Clone();//jobtrips is in descending order based on the sql query
                                newLegTrip.JobID = this.JobID;
                                newLegTrip.Sequence = 0; //auto increment in SP
                                //newLegTrip.PartnerJobNo = this.PartnerJobNo;
                                ...
                                newLegTrip.TripStatus = JobTripStatus.Ready;
                                newLegTrip.JobTripStates = new SortableList<JobTripState>();
                                //20141101 - gerry added
                                newLegTrip.LegType = Leg_Type.OneLeg; //set to 1 leg
                                ...
                                if (this.jobTrips.Count > 0)
                                {
                                    ...
                                    if (this.jobTrips[this.jobTrips.Count - 1].EndStop == stop)
                                    {
                                        if (this.jobTrips[this.jobTrips.Count - 1].StartStop != tripsInSameContainerNo[tripsInSameContainerNo.Count - 1].EndStop)
                                            newLegTrip.StartStop = ((HaulierJobTrip)this.jobTrips[this.jobTrips.Count - 1].Clone()).StartStop;
                                        else
                                            newLegTrip.StartStop = ((HaulierJobTrip)this.jobTrips[this.jobTrips.Count - 1].Clone()).EndStop;
                                    }
                                    else
                                    {
                                        newLegTrip.StartStop = ((HaulierJobTrip)this.jobTrips[this.jobTrips.Count - 1].Clone()).EndStop;
                                    ...
                                    SortableList<HaulierJobTrip> newTripsSameConatiner = GetJobTripsInTheSameContainerNo(this.JobTrips, containerNo);
                                ...
                                foreach (HaulierJobTrip tempTrip in sampleTrips)
                                {
                                    ...
                                    if (tempTrip.EndStop.Code == stop.Code && tempTripContSize == newTripContSize)
                                    {
                                        ...
                                        newLegTrip.PartnerJobNo = tempTrip.JobNo;
                                        newLegTrip.IsLaden = tempTrip.IsLaden;
                                        newLegTrip.StartStop = tempTrip.StartStop;
                                        newLegTrip.EndStop = tempTrip.EndStop;
                                        newLegTrip.TripStatus = JobTripStatus.Ready;
                                        break;
                                    }
                                }
                                ...
                                //this.JobTrips.Add(newLegTrip);
                                maxLegGroup++;
                                newLegTrip.LegGroup = maxLegGroup;
                            }
                            if (newLegTrip != null)
                                allNewTrips.Add(newLegTrip);
                        }
                        ...
                        //AddHaulierJobTrips(this, this.JobTrips, frmName, userID, con, tran);
                        AddHaulierJobTrips(this, allNewTrips, frmName, userID, con, tran);
                        tran.Commit();
                    }
                    else
        ...
        public bool TransferHubbingJobTripsToNewJob(HaulierJob hubbingJob, SortableList<HaulierJobTrip> sampleTrips, string frmName, string userID, string quotationNo, bool isBillByTransport)
        {
            string prefix = HaulierJobDAL.GetPrefix();
            SqlConnection con = new SqlConnection(FMGlobalSettings.TheInstance.getConnectionString());
            SqlTransaction tran = null;
            try
            {
                ...
                {                    
                    ...
                    if (this.ValidateAddHaulierJobHeader())   //job trip validation was transfered in the job trip loop
                    {
                        ...
                        List<string> selectedContainerNos = new List<string>();
                        foreach(HaulierJobTrip trip in sampleTrips)
                        {
                            if (!selectedContainerNos.Contains(trip.ContainerNo))
                                selectedContainerNos.Add(trip.ContainerNo);
                        }                        
                        HaulierJobDAL.UpdatePartnerTrips(hubbingJob.JobID, selectedContainerNos, this.JobNo, con, tran);
                        //now add new trip to database
                        AddHaulierJobTrips(this, sampleTrips, frmName, userID, con, tran);
                        tran.Commit();
                    }
                    else
                        throw new FMException(TptResourceBLL.ErrNoSelectedContainerToTransfer); //"Please select containers to be transferred and where is the next stop. ");
        ...
        public static SortableList<HaulierJobTrip> AddSampleJobTrips_old(HaulierJob oldHaulierJob, List<string> containerNos, List<Stop> additionalStops, bool isBillByTransport, string quotNo)
        {
                ...
                if (containerNos.Count > 0 && additionalStops.Count > 0)
                {
                    ...
                    foreach (Stop stop in additionalStops)
                    {
                        ...
                        foreach (string contNo in containerNos)
                        {
                            ...
                            Stop defaultStartStop = tripsInSameContainerNo[tripsInSameContainerNo.Count - 1].EndStop;
                            ...
                            //20141101 - gerry added
                            sampleTrip.LegType = Leg_Type.OneLeg; //set to 1 leg
        ...
        public SortableList<HaulierJobTrip> AddSampleJobTrips(HaulierJob hubbingJob, List<HaulierJobTrip> hubbingJobContainers, List<Stop> additionalStops, bool isBillByTransport, string quotNo)
        {
            SortableList<HaulierJobTrip> sampleTrips = new SortableList<HaulierJobTrip>();
            try
            {
                if (hubbingJobContainers.Count > 0 && additionalStops.Count > 0)
                {
                    this.PartnerJobTrips = new SortableList<HaulierJobTrip>();
                    foreach (HaulierJobTrip trip in hubbingJob.jobTrips)
                    {
                        this.PartnerJobTrips.Add((HaulierJobTrip)trip.Clone());
                    }
                    int maxLegGroup = this.JobTrips.AsEnumerable().Max(jt => jt.LegGroup);
                    Quotation quotation = Quotation.GetAllQuotationHeader(quotNo);
                    foreach (HaulierJobTrip hubbingJobTrip in hubbingJobContainers)
                    {
                        string contSize = TransportFacadeIn.GetContainerSizeByContainerCode(hubbingJobTrip.ContainerCode);
                        int maxlegGroupMember = GetJobTripsInTheSameContainerNo(this.PartnerJobTrips, hubbingJobTrip.ContainerNo).Count();
                        maxLegGroup++;
                        int stopCount = 0;
                        foreach (Stop stop in additionalStops)
                        {
                            maxlegGroupMember++;
                            HaulierJobTrip newTrip = hubbingJobTrip.Clone() as HaulierJobTrip;
                            newTrip.JobID = this.JobID;
                            newTrip.JobNo = this.JobNo;
                            newTrip.StartStop = hubbingJobTrip.EndStop;
                            if (stopCount > 0 && sampleTrips.Count > 0)
                            {
                                if (sampleTrips[sampleTrips.Count - 1].EndStop == stop)
                                {
                                    if (sampleTrips[sampleTrips.Count - 1].StartStop != hubbingJobTrip.EndStop)
                                        newTrip.StartStop = sampleTrips[sampleTrips.Count - 1].StartStop;
                                    else
                                        newTrip.StartStop = sampleTrips[sampleTrips.Count - 1].EndStop;
                                }
                                else
                                    newTrip.StartStop = sampleTrips[sampleTrips.Count - 1].EndStop;
                            }
                            newTrip.EndStop = stop;
                            newTrip.TripStatus = stopCount == 0 ? JobTripStatus.Ready : JobTripStatus.Booked;
                            newTrip.isBillable = true;
                            newTrip.isMovementJob = true;
                            newTrip.isMulti_leg = true;
                            newTrip.IsLaden = true;
                            newTrip.LegType = Leg_Type.OneLeg;
                            newTrip.Remarks = contSize;
                            newTrip.JobTripStates = new SortableList<JobTripState>();
                            //20141101 - gery added to check the quotation
                            if (isBillByTransport && !quotNo.Equals(string.Empty))
                            { 
                                //first check the old trip if there is valid rate
                                CheckValidQuotaionRates(quotation, contSize, hubbingJobTrip);
                                //2nd check the sample trip or the additional trip if there is valid rate
                                CheckValidQuotaionRates(quotation, contSize, newTrip);
                            }
                            newTrip.LegGroup = maxLegGroup;
                            newTrip.LegGroupMember = maxlegGroupMember;
                            newTrip.SetPartnerJobNo(hubbingJob.JobNo);
                            sampleTrips.Add(newTrip);
                            stopCount++;
                        }                        
                    }
                }
        ...
        private static void CheckValidQuotaionRates(Quotation quotation, string contSize, HaulierJobTrip sampleTrip)
        {
            try
            {
                ...
                {
                    if (sampleTrip.LegType != Leg_Type.OneLeg)
                    {
        ...
        public bool CanUpdateLocationFromHeader(out string warnMsg)
        {
            ...
            try
            {
                ...
                var listMoreThan2LegsTrips = this.JobTrips.Where(jt => jt.LegGroupMember > 2).ToList();
        ...
        public static HaulierJob GetHubbingJob(string customerCode, string containerNo)
        {
            return HaulierJobDAL.GetHubbingJob(customerCode, containerNo);
        }
      
        public static List<string> GetHubbingJobNos(SeaJob seaJob)
        {
            List<string> retValue = new List<string>();
            try
            {
                List<string> containerNoWithoutHubbingJobNo = new List<string>();
                foreach (SeaJobContainer container in seaJob.SeaJobContainers)
                {
                    if (container.ContainerNumber != string.Empty)
                    {
                        string hubbingJobNo = HaulierJobDAL.GetHubbingJobNo(seaJob.CustomerCode, container.ContainerNumber);
                        if (hubbingJobNo != string.Empty)
                        {
                            if (!retValue.Contains(hubbingJobNo))
                                retValue.Add(hubbingJobNo);
                        }
                        else
                            containerNoWithoutHubbingJobNo.Add(container.ContainerNumber);
                    }
                }
                if (retValue.Count > 0 && containerNoWithoutHubbingJobNo.Count > 0)
                {
                    string err = "Seafreight has container number without any hubbing job. \n";
                    err += String.Join(", ", containerNoWithoutHubbingJobNo.ToArray());
                    throw new FMException(err);
                }
            }
            catch (FMException ex) { throw ex; }
            catch (Exception ex) { throw new FMException(ex.Message.ToString()); }
        ...
        public static string[,] GetContainerNoWithHubbingJobNos(SeaJob seaJob)
        {
            var contNos = seaJob.SeaJobContainers.Where(c => c.ContainerNumber != string.Empty).ToList();
            string[,] retValue = new string[contNos.Count, 2];
            try
            {
                for (int i = 0; i < contNos.Count; i++)
                {
                    if (contNos[i].ContainerNumber != string.Empty)
                    {
                        string hubbingJobNo = HaulierJobDAL.GetHubbingJobNo(seaJob.CustomerCode, contNos[i].ContainerNumber);
                        
                        retValue[i, 0] = contNos[i].ContainerNumber;
                        retValue[i, 1] = hubbingJobNo;
                    }
                }
            }
            catch (FMException ex) { throw ex; }
        ...
        public static string[,] GetHubbingJobWithContainerCount(SeaJob seaJob)
        {
            string _20FT = "20FT";
            string _40FT = "40FT";
            var contNos = seaJob.SeaJobContainers.Where(c => c.ContainerNumber != string.Empty).ToList();
        ...
        public static Dictionary<string, string> GetFreshContainerQuantityOfEachSize(SeaJob seaJob)
        {
            string TWENTYFOOT = "20'";
            string FORTYFOOT = "40'";
            string FORTYFIVEFOOT = "45'";
            string OTHERSIZE = "OTH";
            ...
            foreach (SeaJobContainer container in seaJob.SeaJobContainers)
            {
                ...
                {                       //reason : previously 40HC consider as OTH size, so 40HC will not be captured
                    if (container.ContainerCode.Contains(TWENTYFOOT.Substring(0, 2))) { n20++; }
                    if (container.ContainerCode.Contains(FORTYFOOT.Substring(0, 2))) { n40++; }
                    if (container.ContainerCode.Contains(FORTYFIVEFOOT.Substring(0, 2))) { n45++; }
                    if (container.ContainerCode.Equals(OTHERSIZE)) { nOTH++; }
                }
            }
        ...
        public static Dictionary<string, string> GetHubbingContainerQuantityOfEachSize(SeaJob seaJob)
        {
            string TWENTYFOOT = "20'";
            string FORTYFOOT = "40'";
            string FORTYFIVEFOOT = "45'";
